...
# CWE-259 Hardcoded Sensitive Data detector
...
FINDINGS=$(mktemp /tmp/cwe259_findings_XXXXXX)

{
    echo "========================================"
    echo " CWE-259 Hardcoded Sensitive Data Detector"
    echo " Source  : $SRC"
    ...
    bash "$DETECTORS_DIR/detect_password_literal.sh"  "$XML" "$SRC" "$FINDINGS"
    bash "$DETECTORS_DIR/detect_define_credential.sh"  "$XML" "$SRC" "$FINDINGS"
    bash "$DETECTORS_DIR/detect_strcmp_hardcoded.sh"   "$XML" "$SRC" "$FINDINGS"
