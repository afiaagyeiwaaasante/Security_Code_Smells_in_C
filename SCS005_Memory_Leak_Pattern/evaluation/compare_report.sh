...
print_header("SCS005 CWE-401  TIER 1 — Smell Pattern Variants")
for t in tier1:
    print_row(t)
print(LINE)

if tier3:
    print_header("SCS005 CWE-401  TIER 3 — Known Limitation Cases (expected: MISSED)")
    for t in tier3:
        print_row(t)
    print(LINE)

# TP/TN/FP/FN — Tier 1 only
eval_tests = {t: r for t, r in our.items() if r.get("tier") == "tier1"}
...
print()
print("=" * len(HDR))
print("  Metrics (Tier 1 only; Tier 3 limitation cases excluded)")
print("=" * len(HDR))
