...
print("  SCS005 CWE-401  Tool Comparison: Our Tool vs cppcheck vs Joern")
print("=" * len(header_tool))
print(header_sub)
print(header_tool)
print(LINE)
...
for test in all_tests:
    ...
    c = cpp.get(test, {})
    j = joern.get(test, {})
    print(f"{test:<36} | {fmt_time(o.get('wall_time_s')):>7} {fmt_mem(o.get('peak_rss_kb')):>9} {fmt_det(o.get('detected')):>5} | {fmt_time(c.get('wall_time_s')):>7} {fmt_mem(c.get('peak_rss_kb')):>9} {fmt_det(c.get('detected')):>5} | {fmt_time(j.get('wall_time_s')):>7} {fmt_mem(j.get('peak_rss_kb')):>9} {fmt_det(j.get('detected')):>5}")
...
print(f"  Detections    : Our Tool {our_tp+our_fp}/{total}   cppcheck {cpp_tp+cpp_fp}/{total}   Joern {joern_tp+joern_fp}/{total}")
