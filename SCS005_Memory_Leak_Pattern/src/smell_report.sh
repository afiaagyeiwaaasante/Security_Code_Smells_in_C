...
FINDINGS=$(mktemp /tmp/cwe401_findings_XXXXXX)

{
    echo "========================================"
    echo " CWE-401 Memory Leak Detector"
    echo " Source  : $SRC"
    ...
    bash "$DETECTORS_DIR/detect_no_free_on_exit.sh"  "$XML" "$SRC" "$FINDINGS"
    bash "$DETECTORS_DIR/detect_overwrite_leak.sh"   "$XML" "$SRC" "$FINDINGS"
    bash "$DETECTORS_DIR/detect_new_no_delete.sh"    "$XML" "$SRC" "$FINDINGS"
