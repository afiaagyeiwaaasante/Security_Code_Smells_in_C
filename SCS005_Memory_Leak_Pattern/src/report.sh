...
print(f"{'='*48}")
print(f" CWE-401 Memory Leak Detection Report")
...
for finding in findings:
    ...
    if detector == "overwrite_leak":
        msg = f"Memory leak: '{varname}' overwritten without free"
    elif detector == "new_no_delete":
        msg = f"Memory leak: '{varname}' allocated with new, never deleted"
    else:
        msg = f"Memory leak: '{varname}' allocated but never freed"
