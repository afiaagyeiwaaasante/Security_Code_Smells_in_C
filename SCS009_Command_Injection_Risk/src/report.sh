...
print(f"{'='*52}")
print(f" CWE-78 Command Injection Risk Report")
...
DETECTOR_MSGS = {
    "system_tainted": "system() called with user-controlled input — no sanitisation (fgets/getenv source in same block)",
    "popen_tainted":  "popen() called with user-controlled input — no sanitisation (fgets/getenv source in same block)",
    "execl_tainted":  "execl()/execlp() called with user-controlled input — no sanitisation",
}
...
for finding in findings:
    ...
    msg = DETECTOR_MSGS.get(detector, "OS command injection risk")
