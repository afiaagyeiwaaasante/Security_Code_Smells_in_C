...
# CWE-78 Command Injection Risk detector
...
FINDINGS=$(mktemp /tmp/cwe78_findings_XXXXXX)

{
    echo "========================================"
    echo " CWE-78 Command Injection Risk Detector"
    echo " Source  : $SRC"
    ...
    bash "$DETECTORS_DIR/detect_system_tainted.sh" "$XML" "$SRC" "$FINDINGS"
    bash "$DETECTORS_DIR/detect_popen_tainted.sh"  "$XML" "$SRC" "$FINDINGS"
    bash "$DETECTORS_DIR/detect_execl_tainted.sh"  "$XML" "$SRC" "$FINDINGS"
