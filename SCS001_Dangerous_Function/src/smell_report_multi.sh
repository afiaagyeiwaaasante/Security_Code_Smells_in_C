...
# CWE-242 Dangerous Function detector — multi-file variant
# Combines multiple source files into one srcML archive before running detectors.
# Use this for interprocedural cases where gets() is defined in a different file
# from the caller.
set -e
...
# Use first file's directory for intermediate files
DIR=$(dirname "$1")
BASE="combined"
XML="$DIR/$BASE.xml"
...

{
    ...
    # -----------------------------------------------------------------------
    # Run detector against the combined XML
    # -----------------------------------------------------------------------
    bash "$DETECTORS_DIR/detect_dangerous_function.sh" "$XML" "$1" "$FINDINGS"
