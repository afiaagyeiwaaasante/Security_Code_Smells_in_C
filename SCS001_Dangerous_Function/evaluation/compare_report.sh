...
print("  SCS001 CWE-242  Tool Comparison: Our Tool vs cppcheck vs Joern")
print("=" * len(header_tool))
print(header_sub)
print(header_tool)
print(LINE)
...
for test in all_tests:
    ...
    o_time = fmt_time(o.get("wall_time_s"))
    o_mem  = fmt_mem(o.get("peak_rss_kb"))
    o_det  = fmt_det(o.get("detected"))

    c_time = fmt_time(c.get("wall_time_s"))
    c_mem  = fmt_mem(c.get("peak_rss_kb"))
    c_det  = fmt_det(c.get("detected"))

    j_time = fmt_time(j.get("wall_time_s"))
    j_mem  = fmt_mem(j.get("peak_rss_kb"))
    j_det  = fmt_det(j.get("detected"))

    print(f"{test:<36} | {o_time:>7} {o_mem:>9} {o_det:>5} | {c_time:>7} {c_mem:>9} {c_det:>5} | {j_time:>7} {j_mem:>9} {j_det:>5}")
...
print(f"  Detections    : Our Tool {our_det}/{total}   cppcheck {cpp_det}/{total}   Joern {joern_det}/{total}")
if our_times and cpp_times and joern_times:
    print(f"  Avg wall time : Our Tool {sum(our_times)/len(our_times):.3f}s   cppcheck {sum(cpp_times)/len(cpp_times):.3f}s   Joern {sum(joern_times)/len(joern_times):.3f}s")
if our_mems and cpp_mems and joern_mems:
    print(f"  Avg peak RSS  : Our Tool {sum(our_mems)/len(our_mems)/1024:.1f} MB   cppcheck {sum(cpp_mems)/len(cpp_mems)/1024:.1f} MB   Joern {sum(joern_mems)/len(joern_mems)/1024:.1f} MB")
print()
