...
# CWE-190 Integer Overflow Risk detector
...
FINDINGS=$(mktemp /tmp/cwe190_findings_XXXXXX)

{
    echo "========================================"
    echo " CWE-190 Integer Overflow Risk Detector"
    echo " Source  : $SRC"
    ...
    bash "$DETECTORS_DIR/detect_unchecked_multiply.sh"  "$XML" "$SRC" "$FINDINGS"
    bash "$DETECTORS_DIR/detect_unchecked_add.sh"       "$XML" "$SRC" "$FINDINGS"
    bash "$DETECTORS_DIR/detect_unchecked_increment.sh" "$XML" "$SRC" "$FINDINGS"
