...
print(f"{'='*52}")
print(f" CWE-134 Missing Format Specifier Report")
...
DETECTOR_MSGS = {
    "printf_direct":  "printf/vprintf called with variable as format argument — no literal format specifier",
    "fprintf_direct": "fprintf/vfprintf called with variable as format argument — no literal format specifier",
    "syslog_direct":  "syslog called with variable as format argument — no literal format specifier",
}
...
for finding in findings:
    ...
    msg = DETECTOR_MSGS.get(detector, "Missing format specifier")
