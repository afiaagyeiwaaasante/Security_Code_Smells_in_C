...
# CWE-134 Missing Format Specifier detector
...
FINDINGS=$(mktemp /tmp/cwe134_findings_XXXXXX)

{
    echo "========================================"
    echo " CWE-134 Missing Format Specifier Detector"
    echo " Source  : $SRC"
    ...
    bash "$DETECTORS_DIR/detect_printf_direct.sh"  "$XML" "$SRC" "$FINDINGS"
    bash "$DETECTORS_DIR/detect_fprintf_direct.sh" "$XML" "$SRC" "$FINDINGS"
    bash "$DETECTORS_DIR/detect_syslog_direct.sh"  "$XML" "$SRC" "$FINDINGS"
