...
SCS005–SCS009 are expressed entirely in srcQL + XPath — no Python. SCS008 and SCS009 both use srcQL to scope each detector to its matching function body, then apply two sequential XPath checks on the scoped result: (1) the critical argument is non-literal, and (2) a taint source (`fgets`, `getenv`, `scanf`, etc.) is present in the same function. Because srcQL pre-scopes the result, the taint check is a flat `count()` rather than the `ancestor::` predicate that pure XPath requires. SCS010 is **pure XPath only — srcQL is not applicable**: `#define` declarations exist outside any function body so the `FIND $T $FUNC() {} CONTAINS ...` form cannot match them, and credential keyword detection requires case-insensitive substring matching via `translate()` which srcQL does not support. XPath's `translate()` function replaces Python's `re.compile(r'(?i)...')` for credential keyword scanning.
